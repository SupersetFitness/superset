$(document).ready(function() {
  ...
  const testIfEventIsInProximityWindow = (currentLocation, theEvent, proximityWindowInMiles) => { // WORKS
  ...
  const returnArrayOfAllEventsWithinProximityWindow = (currentLocation, proximityWindowInMiles) => { // WORKS
  ...
  const = returnArrayOfEventsAfterTime(targetTime, arrayToInspect) => { // need to test
    const returnedArray = [];
    if (targetTime == "") {
  ...
  const = getDistanceBetweenCoordinatesInMiles(lat1, lon1, lat2, lon2) => { // WORKS
  ...
  const pushEventToDatabase = (eventToPush) => { // Works update for trainer page
...
module.exports = {
  ...
  arrayOfAllEvents: arrayOfAllEvents,
  arrayOfEventKeys: arrayOfEventKeys,
  displayProximateEventsToUserPage: displayProximateEventsToUserPage,
  pushEventToDatabase: pushEventToDatabase,
  currentLocation: currentLocation,
  doesUserHaveActiveTrainer: doesUserHaveActiveTrainer,
  testIfEventIsInProximityWindow: testIfEventIsInProximityWindow,
  returnArrayOfAllEventsWithinProximityWindow: returnArrayOfAllEventsWithinProximityWindow,
  populateDatabaseWithEventInfo: populateDatabaseWithEventInfo,
  putUserEntryInEventObject: putUserEntryInEventObject,
  userSelectedProximity: userSelectedProximity,
  userEvent: userEvent,
  lat1: lat1,
  lat2: lat2,
